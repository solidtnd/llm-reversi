...
- レスポンシブはCSS(flexbox/grid + メディアクエリ)のみで対応し、PC/スマホ専用の別コンポーネント・別ルートには分岐しない。対戦表のような横に長い表は、スマホでは(ページ左右のガターの内側で)横スクロールさせる。表を画面端まで広げる(full-bleed)ことはしない。表示できる幅は増えるが、ページの左端が要素ごとにずれて崩れて見えるため、左右の余白を全要素で揃えることを優先する。
...
- **`scripts/screenshot.mjs`(`npm run screenshot`)はassertionを持たない撮影専用ツール**で、上記の「自動テストは導入しない」方針とは矛盾しない。レイアウト崩れは型チェックやSSRスモークテストでは検出できずブラウザで見るしかないため、スマホ幅を含む複数ビューポートを画像で目視確認するための道具として持つ。`npm run build`・CIには組み込まない。実行にはPlaywright(devDependency)を使うが、追加のブラウザダウンロードを避けるためWindowsに標準搭載の`msedge`チャンネルを使う。
...
- **ページ左右の余白(ガター)は`--gutter`(`clamp(16px, 4vw, 40px)`)に集約し、`.shell`の左右paddingとしてのみ与える。** 幅の基準を1箇所に持つことで、負のマージンで打ち消す等の小細工なしに全要素の左端を揃えられる。
  - `.shell`と併用する他のクラス(`.page`など)で上下の余白を足すときは、**必ず`padding-block`を使い`padding`ショートハンドを使わない。** ショートハンドは左右の値も同時に指定するため、後から適用される側が`.shell`のガターを`0`に上書きしてしまう。
- **数値・座標・対局IDは等幅フォント、見出しはゴシックの太字。** 棋譜(`d3`のような代数記法)を扱う画面なので、桁が揃うことと座標が読みやすいことを優先する。
- **ダークモードは既定で`prefers-color-scheme`に従い、ヘッダーのボタンで手動上書きできる。** 暗い面用の値は明色の単純反転ではなく選び直す(datavizスキルの方針)。手動で切り替えた後はその選択を`localStorage`に保持し、以後は端末設定の変化を追わない。切り替えは`<html data-theme="light"|"dark">`属性で行い、初回描画前に`localStorage`を読んで属性を立てるインラインスクリプトをindex.htmlに置く(再訪問時に一瞬だけ元の配色が見えるのを防ぐため)。
