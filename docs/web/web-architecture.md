...
1. **トップ**: モデルランキング表(**すべての列見出しで並べ替え可**。同じ見出しを再度選ぶと昇降が反転する) + 対戦表(総当たりマトリクス)
   - 絞り込みは**provider(OpenAI/Anthropic/Gemini)単位のみ**とし、モデル名の自由入力検索は持たない。モデル数が十数件の規模では特定のモデル名を探す需要がなく、「この会社のモデルだけ見たい」という需要の方が実際に発生するため。選択肢はデータ(`ranking.json`の`models[].provider`)から作り、providerを決め打ちしない。
   - 絞り込みは対戦表の**行だけ**に効かせ、列は常に全モデルを出す。列も絞ると同一provider同士の対戦しか残らず、「他社のモデルにどう勝ったか」が見えなくなるため。
2. **モデル詳細**(`#/models/<id>`): そのモデルの指標(勝率・先手後手別勝率・反則負け率とその内訳・平均応答時間・平均石数差) + **API利用料(概算)** + 対局履歴(相手モデル・勝敗等で絞り込み可、対局詳細へのリンク付き)
   - 対局履歴では、石数決着の対局はその石数と石数差を出す(僅差だったのか圧倒的だったのかが分かるように)。
3. **対局詳細**(`#/games/<game_id>`): 棋譜リプレイ(盤面 + 手送り操作、その時点の`legal_moves`のうち選ばれなかった手も盤面上に薄く表示) + 手ごとの生ログパネル(`llm_raw_response`・反則理由・トークン数・応答時間) + その対局のトークン数・概算利用料
   - **開いた直後は最終盤面を表示する**(初期配置は全対局で同一で情報がないため)。棋譜を追う場合は手送り操作で戻す。最終盤面から自動再生を押した場合は先頭から再生する。
...
└── src/
    ...
    ├── App.tsx                # ルーティング定義(4ルート)+ 共通レイアウト(ヘッダーのAboutリンク・GitHubリンク・テーマ切り替え、遷移時のスクロール位置リセットを含む)
    ...
    │   ├── GameHistoryTable.tsx   # モデル詳細内の対局履歴(相手モデルで絞り込み可能、石数決着は石数も表示)
    ...
    └── lib/
        ...
        ├── pricing.ts             # モデル別のAPI単価表(下記「API利用料の扱い」参照)
        └── format.ts              # 数値・時刻・金額の表示整形
```
...
- **金額は`data/`のJSONに一切持たせず、Web側の単価表(`lib/pricing.ts`)とトークン数を掛けて表示する。** engine側で算出すると、単価が改定されるたびに集計をやり直して`data/`をコミットし直すことになる(`data/`はリポジトリにコミットする運用)。表示のためだけの数値は表示側で掛ける。
- 単価表は**モデルのid**をキーに、100万トークンあたりのinput/output価格を持つ。値は[../engine/models.md](../engine/models.md)の調査結果(各社の公式pricingページ調べ)を転記する。単価表に載っていないモデルは金額を出さず「単価不明」と表示する(0円として出さない)。
- **単価は変動するため、金額を出す画面には必ず「いつ時点の単価か」と公式料金ページへのリンクを併記する。** 単価表の調査時点は`PRICING_AS_OF`として単価表と同じファイルに持ち、表示側はこれを参照する(時点の文字列を画面ごとに書かない)。
- トークン数自体が概算であること(リトライで失敗した呼び出し分は記録に残らない)も併記する。理由は[../shared/log-schema.md](../shared/log-schema.md#対局結果ログdataresultsjsonl)の`tokens`の項を参照。
...
- **本文の書体は各OS標準のゴシックに任せ、Webフォントは読み込まない。個性的なタイポグラフィは狙わない。** ただしWindowsの既定和文フォントである游ゴシックは字面が小さく字間も詰まって見えるため、同じくWindows標準のメイリオを先に指定して回避する(`--font-sans`)。同じ理由で、和文の字間を詰める`font-feature-settings: "palt"`と、見出しの負の`letter-spacing`は使わない。
- **ページ遷移時は常にページ先頭へスクロールする**(`App.tsx`の`ScrollToTop`)。react-routerはSPA内遷移でスクロール位置を維持するため、対局詳細を下までスクロールしてからランキングへ戻ると戻り先も途中から表示されてしまう。ブラウザの戻る/進む(`POP`)はブラウザ自身の復元に任せて対象外にする。
...
- **ヘッダーにはAboutページへのテキストリンクを常設する。** 指標の定義・対局条件は「表示されている数値の読み方」そのものなので、フッターやトップページ内の導線だけでは辿り着きにくい。狭い画面ではラベルを短縮し(「指標について」)、タイトル・アイコンと合わせて1行に収める(横スクロールが出るとページ全体が横に動いてしまうため)。
