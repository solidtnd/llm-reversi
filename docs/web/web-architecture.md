...
- スクリーンショットは`fullPage`で撮るため、**画像の幅がビューポート幅と一致しているかが横方向のはみ出しの検出になる**(例: 360pxで撮った画像が376pxなら16pxはみ出している)。ページ全体が横スクロールしてしまう不具合は見た目では気づきにくいので、余白がおかしいと感じたらまず`document.scrollWidth`と`clientWidth`の一致を確認する。
...
- **ページ左右の余白(ガター)は`--gutter`(`clamp(16px, 4vw, 40px)`)に集約し、`.shell`の左右paddingとしてのみ与える。** 幅の基準を1箇所に持つことで、負のマージンで打ち消す等の小細工なしに全要素の左端を揃えられる。
  - `.shell`と併用する他のクラス(`.page`など)で上下の余白を足すときは、**必ず`padding-block`を使い`padding`ショートハンドを使わない。** ショートハンドは左右の値も同時に指定するため、後から適用される側が`.shell`のガターを`0`に上書きしてしまう(実際にこの取り違えで、1160px未満の全幅でmain内のコンテンツが画面端に貼り付く不具合が発生した)。
